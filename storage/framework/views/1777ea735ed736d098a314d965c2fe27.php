...
<?php $component->withAttributes([]); ?>
    <div data-page="admin-staff-form" data-api-base="<?php echo e(url('/api/admin/staff')); ?>" data-permissions-url="<?php echo e(url('/api/admin/permissions')); ?>" data-roles-url="<?php echo e(url('/api/admin/roles')); ?>" data-staff-id="<?php echo e($staffId); ?>" data-is-edit="<?php echo e(json_encode($isEdit)); ?>" data-index-url="<?php echo e(route('admin.staff.index')); ?>" data-login-url="<?php echo e(route('login')); ?>" class="p-4 sm:p-6 md:p-8">
        <div class="mb-5 rounded-xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-cyan-400/5 to-cyan-600/10 px-3 py-2 backdrop-blur-sm dark:border-cyan-500/10 sm:px-4 sm:py-2.5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-base font-bold text-transparent dark:from-cyan-400 dark:to-blue-400 sm:text-lg"><?php echo e($isEdit ? 'Edit Staff' : 'Create Staff'); ?></h1>
                    <p class="mt-0.5 flex items-center gap-2 text-slate-600 dark:text-slate-400">
                        <i data-lucide="<?php echo e($isEdit ? 'edit' : 'user-plus'); ?>" class="h-3 w-3"></i>
                        <span><?php echo e($isEdit ? 'Update staff account' : 'Add a new staff member'); ?></span>
                    </p>
                </div>
                <a href="<?php echo e(route('admin.staff.index')); ?>" class="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
            ...
            <form id="staffForm" class="space-y-5">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="password" name="password" id="staffPassword" placeholder="<?php echo e($isEdit ? 'Leave blank to keep current' : 'Min 8 characters'); ?>" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm transition hover:border-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-600 dark:bg-slate-700 dark:text-white">
                ...
                <div class="flex gap-3">
                    <button type="submit" id="submitFormBtn" class="flex-1 rounded-lg bg-cyan-400 px-5 py-2.5 text-center text-sm font-bold text-black transition hover:bg-cyan-500">
                        <?php echo e($isEdit ? 'Update Staff' : 'Create Staff'); ?>

                    </button>
                    <a href="<?php echo e(route('admin.staff.index')); ?>" class="flex-1 rounded-lg border border-slate-300 px-5 py-2.5 text-center text-sm font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
                        Cancel
                    </a>
                </div>
            </form>
...
<?php endif; ?>
<?php /**PATH C:\Users\hp\Documents\Portfolio\car-rental-service\resources\views/admin/staff/form.blade.php ENDPATH**/ ?>