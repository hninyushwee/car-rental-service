...
<?php $component->withAttributes([]); ?>
    <div id="formLayoutWrapper" data-page="admin-vehicle-form" data-edit="<?php echo e($isEdit ? 'true' : 'false'); ?>" data-id="<?php echo e($vehicleId ?? ''); ?>" data-vehicles-api="<?php echo e(url('/api/admin/vehicles')); ?>" data-brands-api="<?php echo e(url('/api/admin/brands')); ?>" data-categories-api="<?php echo e(url('/api/admin/categories')); ?>" data-index-url="<?php echo e(route('admin.vehicles.index')); ?>" data-login-url="<?php echo e(route('login')); ?>" class="p-4 sm:p-6 md:p-8">
        <div class="mb-5 rounded-xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-cyan-400/5 to-cyan-600/10 px-3 py-2 backdrop-blur-sm dark:border-cyan-500/10 sm:px-4 sm:py-2.5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 id="formPageHeader" class="bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-base font-bold text-transparent dark:from-cyan-400 dark:to-blue-400 sm:text-lg"><?php echo e($isEdit ? 'Edit Vehicle Profile' : 'Add New Vehicle'); ?>

                    </h1>
                    <p id="formPageSubheader" class="mt-0.5 flex items-center gap-2 text-slate-600 dark:text-slate-400">
                        <i data-lucide="<?php echo e($isEdit ? 'edit' : 'plus-circle'); ?>" class="h-3 w-3"></i>
                        <span><?php echo e($isEdit ? 'Update this vehicle profile' : 'Add a vehicle to your fleet'); ?></span>
                    </p>
                </div>
                <a href="<?php echo e(route('admin.vehicles.index')); ?>" class="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
            ...
            <form id="vehicleForm" class="space-y-5" enctype="multipart/form-data">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Total Stock</label>
                            <input type="number" name="total_stock" id="vehicleTotalStock" placeholder="e.g., 10" min="0" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm transition hover:border-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-600 dark:bg-slate-700 dark:text-white">
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Available Stock</label>
                            <input type="number" name="available_stock" id="vehicleAvailableStock" placeholder="e.g., 10" min="0" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm transition hover:border-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-600 dark:bg-slate-700 dark:text-white">
                        </div>
                    </div>
                    <div class="mt-4">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <h2 class="mb-4 text-base font-semibold text-slate-900 dark:text-white">Price and photos</h2>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Vehicle photos</label>
                            <div id="imageUploadArea" class="relative overflow-hidden rounded-lg border border-dashed border-slate-300 bg-slate-50 transition hover:border-cyan-500 hover:bg-cyan-50/50 dark:border-slate-600 dark:bg-slate-700/50 dark:hover:border-cyan-500 dark:hover:bg-cyan-950/20">
                                <input type="file" name="images[]" id="imageInput" class="sr-only" accept="image/*" multiple>
                                ...
                                <div id="uploadPlaceholder" class="flex cursor-pointer items-center justify-center gap-3 px-3 py-2.5">
                                    ...
                                    <div class="text-left">
                                        <p class="text-sm font-medium text-slate-700 dark:text-slate-300">Choose photos</p>
                                    </div>
                                </div>
                                    ...
                                    <div class="flex items-center justify-between gap-2 px-3 py-2">
                                        <div class="flex min-w-0 items-center gap-2">
                                            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-cyan-100 dark:bg-cyan-900/50">
                                                <i data-lucide="images" class="h-4 w-4 text-cyan-600 dark:text-cyan-400"></i>
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <p id="fileNamesDisplay" class="truncate text-xs font-medium text-slate-700 dark:text-slate-300"></p>
                                                <p id="fileCountDisplay" class="text-[10px] text-slate-500 dark:text-slate-400"></p>
                                            </div>
                                        </div>
                                        <button type="button" id="removeImageBtn" class="shrink-0 rounded-lg p-1 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/50 dark:hover:text-red-400" title="Remove photos">
                ...
                <div class="flex gap-3">
                    <button type="submit" id="submitFormBtn" class="flex-1 rounded-lg bg-cyan-400 px-5 py-2.5 text-center text-sm font-bold text-black transition hover:bg-cyan-500">
                        <?php echo e($isEdit ? 'Update Vehicle' : 'Create Vehicle'); ?>
                    ...
                    <article class="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-inner dark:border-slate-700 dark:bg-slate-900">
                        <div class="relative flex aspect-video items-center justify-center overflow-hidden bg-gradient-to-br from-cyan-400 to-blue-600">
                            ...
                            <span id="previewBadge" class="absolute right-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-semibold text-slate-800 shadow-sm backdrop-blur-sm dark:bg-slate-800/90 dark:text-slate-200">
                                <?php echo e($isEdit ? 'Updating' : 'New'); ?>
                        ...
                        <div class="p-4">
                            ...
                            <div class="mt-4 flex items-center justify-between border-t border-slate-200 pt-3 dark:border-slate-700">
                                <div>
                                    ...
                                    <p id="previewPrice" class="text-base font-black text-slate-950 dark:text-white">MMK 0.00</p>
                                </div>
...
<?php endif; ?>
<?php /**PATH C:\Users\hp\Documents\Portfolio\car-rental-service\resources\views/admin/vehicles/create.blade.php ENDPATH**/ ?>