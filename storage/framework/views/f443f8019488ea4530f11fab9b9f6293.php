    ...
    <div class="owl-carousel fleet-carousel">
      ...
      <div class="group rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden">
        <div class="relative overflow-hidden h-44 bg-slate-100 dark:bg-slate-800">
          <img class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" src="<?php echo e($imgUrl); ?>" alt="<?php echo e($v->brand->name ?? ''); ?> <?php echo e($v->model); ?>" />
        ...
        <div class="p-3">
          <div class="flex items-center justify-between mb-3">
            <div>
              <h3 class="text-base font-bold text-slate-900 dark:text-white"><?php echo e($v->brand->name ?? ''); ?> <?php echo e($v->model); ?></h3>
              <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium"><?php echo e($v->category->name ?? 'Standard'); ?></p>
            </div>
            <div class="flex items-center gap-1 text-xs font-medium text-slate-600 dark:text-slate-400">
              <i data-lucide="map-pin" class="h-3.5 w-3.5"></i>
              <?php echo e($v->location ?? 'Yangon'); ?>
          ...
          <div class="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-3">
            <div>
              <p class="font-black text-slate-950 dark:text-white">MMK <?php echo e(number_format($v->price_per_day)); ?></p>
      ...
      <article class="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-xl hover:border-cyan-300 flex flex-col justify-between">
        <div>
          <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-50 text-cyan-600 transition-all duration-300 group-hover:bg-cyan-100 group-hover:scale-110">
            <i data-lucide="<?php echo e($service['icon']); ?>" class="h-6 w-6"></i>
          </div>
          <h3 class="mt-5 text-lg font-black text-slate-900 group-hover:text-cyan-600 transition-colors"><?php echo e($service['title']); ?></h3>
          <p class="mt-3 text-sm leading-6 text-slate-600"><?php echo e($service['desc']); ?></p>
        </div>
...
<section class="bg-slate-100 py-12 fade-up">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      ...
      ?>
      <?php $__empty_1 = true; $__currentLoopData = $drivingLicenseTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <?php
      ...
      ?>
      <div class="group rounded-2xl border-2 <?php echo e($colors[2] ?? 'border-yellow-200'); ?> dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col">
        <div class="bg-gradient-to-r <?php echo e($colors[0] ?? 'from-yellow-400'); ?> <?php echo e($colors[1] ?? 'to-yellow-500'); ?> px-4 py-4 text-white relative overflow-hidden">
          ...
          <div class="relative z-10">
            <h3 class="text-lg font-bold"><?php echo e($lt->type); ?></h3>
            <?php if($desc): ?>
            <p class="mt-0.5 text-xs text-white/80 font-medium"><?php echo e($desc); ?></p>
            <?php endif; ?>
          </div>
        </div>
        <div class="p-4 flex-1 flex flex-col">
          ...
          <div class="mb-3 -mx-4 -mt-4 h-32 overflow-hidden">
            <img src="<?php echo e($imgUrl); ?>" alt="<?php echo e($lt->type); ?>" class="h-full w-full object-cover" loading="lazy">
          </div>
          <?php endif; ?>
          <div class="flex items-center gap-2 mb-3">
            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold <?php echo e($colors[3] ?? 'bg-yellow-100'); ?> <?php echo e($colors[4] ?? 'text-yellow-700'); ?> dark:bg-slate-800 dark:text-slate-300">
              ...
              <?php echo e($lt->drivers_count ?? 0); ?> driver<?php echo e($lt->drivers_count !== 1 ? 's' : ''); ?> available
            </span>
          </div>
          <div class="mt-auto flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-3">
            <div>
              <p class="font-black text-slate-950 dark:text-white text-sm">MMK <?php echo e(number_format($lt->price ?? 0)); ?></p>
...
<?php endif; ?> 
<?php /**PATH C:\Users\hp\Documents\Portfolio\car-rental-service\resources\views/user/home.blade.php ENDPATH**/ ?>