<x-admin.layout>
    <div data-page="admin-promotion-form" data-api-base="{{ url('/api/admin/promotions') }}"
         data-promotion-id="{{ $promotionId }}" data-is-edit="{{ json_encode($isEdit) }}"
        ...
        <div class="mb-5 rounded-xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 via-cyan-400/5 to-cyan-600/10 px-3 py-2 backdrop-blur-sm dark:border-cyan-500/10 sm:px-4 sm:py-2.5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-base font-bold text-transparent dark:from-cyan-400 dark:to-blue-400 sm:text-lg">{{ $isEdit ? 'Edit Promotion' : 'Create Promotion' }}
                    </h1>
                    <p class="mt-0.5 flex items-center gap-2 text-slate-600 dark:text-slate-400">
                        <i data-lucide="tag" class="h-3 w-3"></i>
                        <span>{{ $isEdit ? 'Update promotion details' : 'Create a new coupon or discount' }}</span>
                    </p>
                </div>
                <a href="{{ route('admin.promotions.index') }}" class="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
